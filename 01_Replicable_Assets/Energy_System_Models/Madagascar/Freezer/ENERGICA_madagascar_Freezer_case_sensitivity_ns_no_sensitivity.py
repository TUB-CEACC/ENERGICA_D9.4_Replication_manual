...
Comments:
    ...
    + Freezer operation: fixed time series as supply
...
# custom function 
from madagascar_economic_fncs_Freezer_case import (number_of_investment,
                                   residual_value,
                                   Capex,
                                   CRF_fnc,
                                   annuity,
...
from system_results_fncs_freezer_case import (sizing_fnc,
...
File_name = 'Results_freezer_high_wntl.xlsx'
...
file_name_operation_behaviour=  'Results_freezer_high_wntl_opb.xlsx'  
sheet_name_OPB='OPB'
...
input_filename = os.path.join(os.getcwd(), "Madagascar_input_Freezer_case.xlsx")
feedin = pd.read_excel(input_filename,sheet_name='INPUT' ) 

demand_normalized_freezer=feedin["Freezer_demand_normalized"]
demand_freezer= feedin["Freezer_demand"]
...
#########
#scenario 1 representative consumer composition [1,3,1]
#if scenario == 1:
...
Comp_param = {
    ...
    'freezer':{
                  'status'                :'on',
                  'LHV'                   : 0,        # Low Heat Value in kWh/l
                  'fix'                   : 1220,        # Fix investment costs in EUR  (independent of the size_comp)
                  'fuel_price'            : 0,        # EUR/l
                  'fuel_escalation_rate'  : 0,        # fuel_escalation_rate in percent 
                  'var'                   : 0,        # Utilisation costs in EUR/kWh 
                  'c_inv'                 : 0,      # Investment cost EUR/kW
                  'c_inv_kWh'             : 0,        # Investment cost EUR/kWh
                  'o&m'                   : 0,       # OPEX in EUR/kW/a  
                  'life_time'             : 10,       # Component's life time 
                  'efficiency'            : 0,        # 
                       }, 
...
#                   'fuel_escalation_rate'  : 0,        # fuel_escalation_rate in percent 
#                   'var'                   : 0,        # Utilisation costs in EUR/kWh 
#                   'c_inv'                 : 830,      # Investment cost EUR/kW
#                   'c_inv_kWh'             : 0,        # Investment cost EUR/kWh
#                   'o&m'                   : 366,       # OPEX in EUR/kW/a  --> switch to €/y?
#                   'life_time'             : 10,       # Component's life time 
#                   'efficiency'            : 0,33        # %
#                        } 
                    }   
...
#controller
#epc_controller = economics.annuity(500, 20, 0.01)
...
freezer = solph.Transformer(
    label="freezer",
    inputs={dc_b2:solph.Flow(investment=solph.Investment(
        ep_cost=cost_param.loc['Annuity_'+project_param['currency'] +'/kW','freezer']))},
...
ES_base_case.add( controller, freezer)        
...
freezer_demand = solph.Sink(label = "freezer_demand", inputs = {f_b1: solph.Flow(fix=demand_normalized_freezer, nominal_value= demand_freezer.max()/1000)}) 
# husks_sink = solph.Sink(label ="husks_sink", inputs ={r_b3: solph.Flow()}) # simple sink, no revenue for husks?




ES_base_case.add(demand_el_1, demand_el_2, demand_el_3, demand_el_4, demand_el_5, excess_el, freezer_demand) #husks_sink
...
f_b1 = solph.views.node(df_results, "freezer_bus_1_label")
...
data_freezer = solph.views.node(df_results, "freezer")

print('data freezer',data_freezer["sequences"].sum(axis=0))             
...
df_freezer= pd.DataFrame(data_freezer["sequences"].sum(axis=0))
...
freezer_bus_1_label='freezer_bus_1_label'
freezer_opt = freezer_optimized ( 'freezer',df_results,model,dc_bus_2_label,  freezer_bus_1_label)
...
e_flows_main=[pv_opt.loc['Energyflow_optimized_kWh','pv'],
         ...
         freezer_opt.loc['Energyflow_optimized_in_kWh','freezer']]
...
#TODO: put the buses here
#TODO: Here is fix required
...
if  operation_behaviour_analysis== 'on':
    ...
    Result_concat['freezer_demand']=f_b1['sequences'][(('freezer_bus_1_label', "freezer_demand"),'flow')]
...
cost_PUE = Cost_system_opt.loc['ANNUITY_EUR','freezer'] # annualized cost of service
...
multiple_dfs(df_list,'Scenario', File_name, 2)
#################################################################################################
