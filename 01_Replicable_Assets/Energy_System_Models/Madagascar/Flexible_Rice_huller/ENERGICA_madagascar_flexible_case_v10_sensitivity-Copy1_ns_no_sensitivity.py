...
Case: Flexible_Case Rice huller

Comments:
    + PV size: variable
    + Battery Capacity: variable
    + Rice Huller operation: minimal annual flow
...
from oemof.solph import (Sink, Transformer, Source, Bus, Flow, NonConvex,
                               Model, EnergySystem, components, custom, Investment, views, constraints)

# from oemof.solph.constraints import integral_limit
from oemof.solph.constraints import generic_integral_limit
...
# custom function 
from madagascar_economic_fncs_flexible_case import (number_of_investment,
                                   residual_value,
                                   Capex,
                                   CRF_fnc,
                                   annuity,
...
File_name = 'Results_flexible_high_wntl.xlsx'
...
file_name_operation_behaviour=  'Results_flexible_high_wntl_opb.xlsx'  
sheet_name_OPB='OPB'
...
input_filename = os.path.join(os.getcwd(), "Madagascar_input_Flexible_case.xlsx")
...
project_param= {
                     ...
                     'wacc'             : 0.1,#wacc,   # Weighted average cost of capital (0<wacc<1) 
                     'currency'         : 'EUR'
               }
...
# rice source
flow_rice = solph.Flow()
rice_source = solph.Source(label = 'rice_source', 
                           outputs={r_b1: flow_rice})
...
rice_huller = solph.Transformer(
    ...
    inputs={dc_b2:solph.Flow(investment=solph.Investment(
        ep_cost=cost_param.loc['Annuity_'+project_param['currency']+'/kW','rice_huller'])),
            r_b1: solph.Flow(nominal_value=400,summed_min=916.40/4)},
    outputs={r_b2: solph.Flow(), r_b3: solph.Flow()},
    conversion_factors={r_b1: 1, dc_b2: 1/Comp_param['rice_huller']['efficiency'], r_b2: 0.55, r_b3: 0.45})   #1/70 kWh/kg    # summed_min definition for minimal annual summed flow
...
# #Constraints 
# flow_with_keyword = {(rice_source, r_b1): flow1, }

# model = generic_integral_limit(model, "my_factor", flow_with_keyword, limit = 100)
...
#we should develop another function to get the already nominal_values we defined, I've not done that before
#I asked in the forum wether they have default function for that
...
###########
####### SAVE IN EXCEL
#File_name = 'PUE_loads.xlsx'
#print(type(dc_b2["sequences"]))
#writer = pd.ExcelWriter(File_name, engine="openpyxl",
#                            mode='a', if_sheet_exists = 'overlay')# as writer:

#dc_b2["sequences"].to_excel(writer, sheet_name='RH_flexible')
...
#     multiple_dfs(df_list,'Scenario_'+str(scenario), File_name, 2)
#################################################################################################
...
#Excel export        
###############################################################################
File_ = 'sensitivity_results.xlsx'
def save_sensi():
    return 'Saved'
