...
## costs_preprocessing
#Oemof (in the investment method) needs the annuity of NPV of each component 
...
def number_of_investment (project_lifetime,component_lifetime): 
    ...
    if project_lifetime == component_lifetime :
        ...
        case 1: 
        round(2.3)= 2, this is no true because investment should be done more than 2 times (2.3), should be 3 times 
        
        case 2: 
        round(2.3+ 0.5)= 3, this true! we need 3 investment
...
def residual_value (c_invest,project_lifetime,component_lifetime,Investment_num,wacc):
      
     #Parameters:
     #Investment_num: number_of_investment needed for each component
     #life time of the component 
     #project duration 
     #c_invest: first investment 
    ...
    if Investment_num* component_lifetime > project_lifetime: 
        
         c_last     = c_invest/(1+wacc)**((Investment_num-1)*component_lifetime)
         
         c_residual =c_last/component_lifetime*(Investment_num*component_lifetime-project_lifetime)
...
def Capex (c_invest,c_residual,project_lifetime,component_lifetime,Investment_num,wacc):
    
    #Parameters:
    #c_invest: specific investment costs [currency/kW]
    #c_residual: residual value [currency/kW]
    #project_lifetime: project duration 
    #n: number of investment 
    #wacc
    ...
    if Investment_num*component_lifetime == project_lifetime:
        ...
        while Replacement_i < Investment_num:
            capex = capex+ c_invest/(1+wacc)**(Replacement_i*component_lifetime)
                
            Replacement_i+=1
    
    
    if  Investment_num*component_lifetime != project_lifetime: #has to be > project lifetime
        Replacement_i=0
            
        while Replacement_i < Investment_num:
            
            capex= capex + c_invest/(1+wacc)**(Replacement_i*component_lifetime)  #Future investment due to replacement discounted to present value and added to capex
                 
            Replacement_i= Replacement_i+ 1
            
        
        capex= capex  - (c_residual/(1+wacc)**project_lifetime)  #Residual value from component after project lifetime is discounted and retrieved from capex (present value)
...
###############################################################################
# Annual costs of each component: annuity which oemof in the investment method will be required  
# Annuity takes into account CAPEX using CRF and O&M in form of OPEX per unit
# The Annuity are not taking account the Utilisation_cost_per_kWh_per_year (Var) 
#CRF ist used calculate the annuity of a present value 
...
def cost_component_optimized (Annuity_per_kW_list
    ...
    #fuel_price_list:      [currency/liter]
    ...
    for i in range(len(component_List)): 
        ...
        Annuity_Optimised_Sizing.append(Capacity_opt_kW_list[i] * Annuity_per_kW_list[i] + Capacity_opt_kWh_list[i] * Annuity_per_kWh_list[i] + Annuity_fix_list[i])
        ...
        Capex_Optimised_Sizing.append(Capacity_opt_kW_list[i] * Capex_per_kW_list[i] +  Capacity_opt_kWh_list[i] * Capex_per_kWh_list[i] + Capex_fix_list[i])
...
def system_opt_result (total_cost_system,
                       total_energy_supply,
    ...
    res_eco ={}
    res_eco['TOTAL_GENERATION_kWh']             = total_energy_production
    res_eco['TOTAL_SUPPLY_kWh']                 = total_energy_supply
    res_eco['Residential_DEMAND_kWh']           = Residential_DEMAND
    res_eco['Service_DEMAND_kWh']               = Service_DEMAND
    
    res_eco['EXCESS_GENERATION_SHARE_%']        = EXCESS_GENERATION_SHARE
    res_eco['EXCESS_HOURS']                     = EXCESS_HOURS
    res_eco['FUEL_CONSUMPTION_LITER']           = FUEL_CONSUMPTION
    res_eco['CO2_Emissions_KG']                 = CO2_Emissions
    res_eco['TOTAL_ANNUALIZED_COST_SYS_'+currency] = total_cost_system 
    res_eco['LCOS_'+currency+'/kWh']                = LCOS
    res_eco['Residential_LCOE_'+currency+'/kWh']    = Residential_LCOE
    res_eco['Service_LCOE_'+currency+'/kWh']        = Service_LCOE 
