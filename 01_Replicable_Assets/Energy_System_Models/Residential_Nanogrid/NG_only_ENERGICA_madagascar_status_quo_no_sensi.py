...
Comments:
    ...
    + No PUE component connected to the nanogrid: therefore the rice source nominal value is set to 0 to shut off the diesel huller.
   
   @Edouard updates: 01/12/2022  
    + modifications name investment functions
    + sensitivity analysis updated
    + demand 1 to 5 (change the input excel data)
    + fix costs included (in economic fct yes, in oemof not)
    + Evaluation Criterias Done  
...
# custom function 
from madagascar_economic_fncs_status_quo import (number_of_investment,
                                   residual_value,
                                   Capex,
                                   CRF_fnc,
                                   annuity,
...
from Madagascar_system_results_fncs_status_quo_v1 import (sizing_fnc,
                                         results_postprocessing,
                                         pv_optimized,
                                         battery_optimized,
                                         controller_optimized,
                                         # rice_huller_optimized,
                                         diesel_huller_optimized,
                                         legend_fnc,
                                         get_energy_controller,
#ToDO: Check if get_energy_controller required
                                         )
                             
         
###############################################################################         
#SAVE IN EXCEL: DEF of scenario

File_name = 'Results_NG_base_high_wntl.xlsx'
...
file_name_operation_behaviour=  'Results_NG_base_high_wntl_opb.xlsx'  
sheet_name_OPB='OPB'
...
#path = "C:\Installation\Logiciel\Python_files\Masterarbeit\\Nanogrid_simulations\Scenario_Madagascar_ED\Rice_case\Status_quo\Graphs_NG"

#C:\Users\nikol\OneDrive\Desktop\Desktop_new\Projekte\ENERGICA\Workpackages\WEP2\models\Status Quo

###############################################################################
  #Sensitivity analysis 
#path and filename to save the sensitivity analysis result
...
solver = 'cbc'
   ...
   #scenario 1 geändert auf 2er Konstellation. Muss eigentlich 0,1,1,1,2

input_filename = os.path.join(os.getcwd(), "Madagascar_input_status_quo.xlsx")
...
Comp_param = {
       ...
       'pv'      :{   
                 ...
                 'life_time'                 : 15,                          # Component's life time  
    ...
    'controller':{  
     ...
     'diesel_huller':{
                  ...
                  'fuel_emission_coef'    : 2.7,      #kg_CO2/L diesel
                  'fuel_price'            : 0.75,        # EUR/l
                  'fuel_escalation_rate'  : 0,        # fuel_escalation_rate in percent 
                  'var'                   : 0,        # Utilisation costs in EUR/kWh 
                  'c_inv'                 : 1.73,      # .Investment cost EUR/(kg_paddy/h)/a
                  'c_inv_kWh'             : 0,        # Investment cost EUR/kWh
                  'o&m'                   : 0.76,       # .OPEX in EUR/(kg_paddy/h)/a
                  'life_time'             : 5,       # Component's life time 
                  'efficiency'            : 0.00512820512820513   # Fuel consumption per paddy transformation L/kg
                       } 
                    }   


###############################################################################
#Add components in component_List, which should be analyzed
...
for i in range(len(component_List)):
    ...
    Investments_Number.append(number_of_investment(project_param['project_lifetime'],
...
r_b3 = solph.Bus(label = 'rice_bus_3_label')
    ...
    #fuel bus
f_b1 = solph.Bus(label = 'fuel_bus_1_label')


ES_base_case.add(dc_b1, dc_b2, r_b1, r_b2, r_b3, f_b1) 
...
fuel_source = solph.Source(label = 'fuel_source',
                           outputs={f_b1: solph.Flow(
                           )})
...
rice_source = solph.Source(label = 'rice_source', 
                           outputs={r_b1: solph.Flow(
                             nominal_value = 0         #switch off the diesel huller
                           )}) 



ES_base_case.add( pv, fuel_source, rice_source)
...
rice_huller = solph.Transformer(
    label="diesel_huller",
    inputs={f_b1:solph.Flow(variable_costs=Comp_param['diesel_huller']['fuel_price']),   
            r_b1:solph.Flow(investment=solph.Investment(
        ep_cost=cost_param.loc['Annuity_'+project_param['currency']+'/kW','diesel_huller']))},#annuity in €/(kg/h)
    outputs={r_b2: solph.Flow(), r_b3: solph.Flow()},
    conversion_factors={r_b1: 1, f_b1: Comp_param['diesel_huller']['efficiency'], r_b2: 0.55, r_b3: 0.45})   #f_b1:0,00512820512820513 L/kg
...
#####FROM HERE on must be transfered in seperated script 
...
f_b1 = solph.views.node(df_results, "fuel_bus_1_label")
...
print(f_b1["sequences"].sum(axis=0))
...
data_rice_huller = solph.views.node(df_results, "diesel_huller")

print(data_pv["sequences"].sum(axis=0))             
...
demand_el_sum = ( demand_normalized_1.sum()* demand_1.max()
                 ...
                 +demand_normalized_5.sum()* demand_5.max())/1000    #coefficient 1000: W to kW
...
power_kWh  = [df_pv.iloc[0,0],
            ...
            df_battery.iloc[2],
            df_excess.iloc[0,0],
            # df_rice_huller.iloc[0,0],   #unit is Liter 
            demand_el_sum]
"""
...
#fuel
fuel_bus_1_label='fuel_bus_1_label'

rice_huller_opt = diesel_huller_optimized ( 'diesel_huller',df_results,model,fuel_bus_1_label, rice_bus_1_label, rice_bus_2_label)
...
fuel_consumption = f_b1['sequences'][(("fuel_source", 'fuel_bus_1_label'), 'flow')].sum()

print('fuel_consumption :', fuel_consumption)
###############################################################################
#Sizing after solving the optimization, DataFrame of all the components
#v4 with new structure according to Odou_no_DG_constraints
...
#sizing_kW_kWh=sizing_kW_kWh.fillna(0)

print('Sizing\n', sizing_kW_kWh)
print('#diesel huller Capacity actually in Production rate: kg_paddy/h')
...
e_flows_main=[pv_opt.loc['Energyflow_optimized_kWh','pv'],
         ...
         rice_huller_opt.loc['Energyflow_optimized_in_kWh','diesel_huller'] #fuel consumption Liter
             ]
...
e_flows_all=[pv_opt.loc['Energyflow_optimized_kWh','pv'],
         ...
         battery_opt.loc['Energyflow_optimized_in_kWh','battery'],
         controller_opt.loc['Energyflow_optimized_in_kWh','controller'],
         rice_huller_opt.loc['Energyflow_optimized_in_kWh','diesel_huller'], 
         # rice_huller_opt.loc['Riceflow_optimized_in_kg','diesel_huller']   
            ]
...
Fuel_consumption_list = [0,0,0,0,0,rice_huller_opt.loc['Energyflow_optimized_in_kWh','diesel_huller']]
...
print('the Fuel_consumption is given in liter (=Energyflow for diesel huller)') 
print('The diesel huller capacity is the max production rate: kg_paddy/h') 



#TODO: All the busses in here...
...
#this is code from previous paper
    ...
    if  operation_behaviour_analysis== 'on':
        ...
        Result_concat= results_postprocessing(df_results,component_List)           
        Result_concat['demand_el']=bel_ac['sequences'][(('ac_electricity', "el_demand"),'flow')]            
        Result_concat['SOC_battery']= data_battery['sequences'][(('battery','nan'),'storage_content')].values / battery_opt.loc['Capacity_optimized_kWh','battery']
        Result_concat['SOC_H2storage']= data_h2storage['sequences'][(('h2storage','nan'),'storage_content')].values / h2storage_opt.loc['Capacity_optimized_kWh','h2storage']
        Result_concat['excess']=bel_dc['sequences'][(('dc_electricity', "excess_el"),'flow')]
    ...
    if operation_behaviour_analysis_plot  == 'on':   
        
        fig, ax = plt.subplots(figsize=(15, 9),dpi=300)
        
        plt.style.use('ggplot')
        
        bel_ac["sequences"][date_from:date_to].plot(ax=ax, kind="line", drawstyle="steps-post", fontsize=20)
        bel_dc["sequences"][date_from:date_to].plot(ax=ax,kind="line", drawstyle="steps-post",linestyle='--',fontsize=20)
        h2_bus["sequences"][date_from:date_to].plot(ax=ax, kind="line", drawstyle="steps-post",linestyle='-',fontsize=20)
        
        ax.legend(loc="upper center", prop={"size": 8}, bbox_to_anchor=(0.5, 1.3), ncol=2,fontsize=40)
        ...
        plt.savefig(path_out+figname_OPB_plot+'.pdf',bbox_inches = 'tight')
...
#     hourly_events.plot.bar(ax=ax[0], legend=False,rot=0)
...
cost_PUE = Cost_system_opt.loc['TOTAL_ANNUITY_EUR','diesel_huller'] # annualized cost of service
...
CO2_Emissions  =  fuel_consumption * Comp_param['diesel_huller']['fuel_emission_coef']  
...
multiple_dfs(df_list,'Scenario_2', File_name, 2)
#################################################################################################
